...
sudo -u postgres psql -d vigie -v ON_ERROR_STOP=1 -c "CREATE TABLE IF NOT EXISTS schema_migrations(filename text PRIMARY KEY, applied_at timestamptz NOT NULL DEFAULT now());"
for sql in "$APP_DIR"/deploy/sql/*.sql; do
  filename="$(basename "$sql")"
  if ! sudo -u postgres psql -d vigie -Atqc "SELECT 1 FROM schema_migrations WHERE filename='$filename'" | grep -q 1; then
    { echo "BEGIN;"; cat "$sql"; printf "\nINSERT INTO schema_migrations(filename) VALUES ('%s');\n" "$filename"; echo "COMMIT;"; } | sudo -u postgres psql -d vigie -v ON_ERROR_STOP=1
  fi
done
