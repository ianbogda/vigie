...
export function EstablishmentsView({all,q,setQ,select,reload}:{all:Eple[];q:string;setQ:(v:string)=>void;select:(id:string)=>void;reload:()=>Promise<void>}){
 ...
 return <div className="page"><div className="pagehead"><div><h2>Établissements</h2><p>Référentiel des EPLE suivis par Vigie.</p></div><div className="est-actions"><div className="search"><Search size={16}/><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Rechercher…"/></div><button className="btn-primary-v" onClick={()=>setEditing(null)}><Plus size={16}/> Ajouter</button></div></div>
 <div className="est-cards">{rows.map(e=><div className="est-row" key={e.id}><button className="est-open" onClick={()=>select(e.id)}><Building2/><div><b>{e.name}</b><small>{e.uai||'UAI non renseigné'} · {e.opaleEntity||'ETS Op@le non renseigné'}</small></div><div className="dots"><StatusDot state={e.states.budget}/><StatusDot state={e.states.financial}/><StatusDot state={e.states.suppliers}/><StatusDot state={e.states.accounting}/></div><ChevronRight/></button>{e.registryId&&<div className="est-tools"><button title="Modifier" onClick={()=>setEditing(registry.find(x=>x.id===e.registryId)||null)}><Pencil size={15}/></button><button title="Supprimer ou archiver" onClick={()=>void remove(e)}><Trash2 size={15}/></button></div>}</div>)}</div>
 {archived.length>0&&<section className="archived-est"><h3><Archive size={16}/> Établissements archivés</h3>{archived.map(e=><div key={e.id}><span><b>{e.name}</b><small>{e.uai} · {e.opale_entity||'sans ETS'}</small></span><button className="btn-soft" onClick={()=>void restore(e)}><RotateCcw size={14}/> Réactiver</button></div>)}</section>}
