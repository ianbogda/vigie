...
export default function App(){
 const[dash,setDash]=useState<Dashboard|null>(null),[selectedId,setSelectedId]=useState('all'),[view,setView]=useState('Accueil'),[modal,setModal]=useState(false),[result,setResult]=useState<any>(null),[openSignal,setOpenSignal]=useState<string|null>(null),[q,setQ]=useState('');
 ...
 {dash?(view==='Accueil'?<HomeView dash={dash} current={current} openSignal={openSignal} setOpenSignal={setOpenSignal}/>:view==='Établissements'?<EstablishmentsView all={all} q={q} setQ={setQ} select={id=>{setSelectedId(id);setView('Accueil')}}/>:<DomainView title={view} current={current}/>):<div className="loading">Construction du cockpit…</div>}</main>
