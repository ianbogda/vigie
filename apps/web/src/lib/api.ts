import type { AgedResponse, ApiMessage, BudgetResponse, FdrAnalysisResponse, FinancialResponse } from '../types/api';
...
export const api = {
  ...
  logout: () => fetch('/api/auth/logout', { method: 'POST' }).then(json<ApiMessage>),
  dashboard: () => fetch('/api/dashboard').then(json<Dashboard>),
  budget: (ets: string, exercise?: number) =>
    fetch(`/api/budget/${encodeURIComponent(ets)}${exercise ? `?exercise=${exercise}` : ''}`).then(
      json<BudgetResponse>
    ),
  ...
  aged: (ets: string, kind: 'clients' | 'suppliers', exercise?: number) =>
    fetch(`/api/aged/${encodeURIComponent(ets)}/${kind}${exercise ? `?exercise=${exercise}` : ''}`).then(
      json<AgedResponse>
    ),
  pcifStatus: () => fetch('/api/integrations/pcif/status').then(json<any>),
  syncPcif: (uais: string[]) =>
    fetch('/api/integrations/pcif/sync', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
  ...
  updateUserAgencies: (id: number, roles: any[]) =>
    fetch(`/api/admin/users/${id}/agencies`, {
      method: 'PUT',
      headers: { 'content-type': 'application/json' },
  ...
  updateAgency: (id: number, body: any) =>
    fetch(`/api/admin/agencies/${id}`, {
      method: 'PUT',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify(body)
    }).then(json<any>),
  setEstablishmentAgency: (id: number, agencyId: number | null) =>
    fetch(`/api/admin/establishments/${id}/agency`, {
      method: 'PUT',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ agencyId })
    }).then(json<any>),
  establishments: () => fetch('/api/establishments').then(json<any>),
  createEstablishment: (body: any) =>
    fetch('/api/establishments', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify(body)
    }).then(json<any>),
  updateEstablishment: (id: number, body: any) =>
  ...
  importOpale: (form: FormData, ets?: string, exercise?: number) => {
    const q = new URLSearchParams();
    if (ets) q.set('ets', ets);
    if (exercise) q.set('exercise', String(exercise));
    return fetch(`/api/import/opale${q.toString() ? `?${q}` : ''}`, { method: 'POST', body: form }).then(json<any>);
  }
};
