...
function InlineFormula({ formula }: { formula: HelpFormula }) {
  ...
  return (
    <div className="indicator-info-equation">
      <span className="indicator-info-fraction">
        <span>{formula.numerator}</span>
        <span>{formula.denominator}</span>
      </span>
      {formula.suffix && <span>{formula.suffix}</span>}
    </div>
  );
}
...
export function IndicatorInfo({ id, label }: { id: IndicatorId; label?: string }) {
  ...
  return (
    <span className="indicator-info" ref={root}>
      <button
        type="button"
        className="indicator-info-trigger"
        aria-label={`Définition : ${label || indicator.name}`}
        aria-expanded={open}
        onClick={(e) => {
          e.stopPropagation();
          setOpen((v) => !v);
      ...
      {open && (
        <span className="indicator-info-popover" role="note" onClick={(e) => e.stopPropagation()}>
          <span className="indicator-info-title">
            <b>{indicator.name}</b>
            {indicator.short && <em>{indicator.short}</em>}
          </span>
          <span className="indicator-info-meaning">{indicator.meaning}</span>
          {indicator.formula && <InlineFormula formula={indicator.formula} />}
          {indicator.reading && (
            <span className="indicator-info-reading">
              <b>Lecture</b>
              {indicator.reading}
            </span>
          )}
          {indicator.caution && (
            <span className="indicator-info-caution">
              <b>Attention</b>
              {indicator.caution}
            </span>
          )}
