...
const monthDate = (v: unknown) => {
  const m = clean(v).match(/^(\d{2})\/(\d{4})$/);
...
export function parseAccountingCsv(buf: Buffer, contextEntity: string) {
  const records: any[] = parseCsv(buf.toString('utf8').replace(/^\uFEFF/, ''), {
    columns: true,
    delimiter: ';',
    bom: true,
  ...
  const parsed = records
    .slice(0, 100000)
    .map((r, i) => {
      const account = value(r, 'Compte').replace(/\s/g, ''),
        period = value(r, 'Période'),
        periodDate = monthDate(period),
      ...
      return {
        ...
        entity,
        account,
        accountLabel: value(r, 'Libellé du compte'),
        journal,
        period,
        periodDate,
        debit: money(value(r, 'Cumul débit')),
        credit: money(value(r, 'Cumul crédit')),
        movementKind: movementKind(journal),
        raw: r
      };
    })
    .filter((x) => /^[1-8]\d*$/.test(x.account) && x.periodDate);
  ...
  if (entities.length > 1)
    throw new Error(
      `Le fichier contient plusieurs ETS (${entities.join(', ')}). Un import comptable doit être rattaché à un seul ETS.`
    );
  ...
  if (fileEntity && fileEntity !== contextEntity)
    throw new Error(
      `ETS incohérent : le fichier contient ${fileEntity}, alors que l’import est contextualisé sur ${contextEntity}.`
    );
  ...
  return {
    ...
    fileEntity,
    entityFromContext: !fileEntity,
    rows: parsed,
    sourceRows: records.length,
    periodFrom: dates[0],
    periodTo: dates.at(-1)!,
    sourceFormat: 'opale-accounting-monthly-summary'
  };
}
