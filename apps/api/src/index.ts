...
app.post('/api/import/opale',async(req:any,reply:any)=>{try{
 ...
 try{
  ...
  return reply.code(400).send({error:'Type Op@le non reconnu. Formats gérés : balance .lis, budget .lis/.xlsx, données comptables Op@le .csv (classes 1 à 8), CLCA .csv et FDR .csv.'});
...
app.get('/api/budget/:ets',async(req:any,reply:any)=>{try{
 ...
 const summary={expenses:metric(dep),revenues:metric(rec),unclassified:metric(other)};
 ...
 return {establishment:{id:establishment.id,uai:establishment.uai,name:establishment.name,opaleEntity:establishment.opale_entity},snapshot:{id:snap.id,date:snap.snapshot_date,filename:snap.source_filename,createdAt:snap.created_at,rowCount:snap.row_count},summary,services,signals};
