...
function parseFdr(buf: Buffer, contextEntity: string) {
  ...
  if(!contextEntity) throw new Error('Le contexte ETS est obligatoire pour un import YFDR.');
  const entities=[...new Set(rows.map((x:any)=>x.establishment).filter(Boolean))];
  if(entities.length>1) throw new Error(`Le fichier YFDR contient plusieurs ETS (${entities.join(', ')}).`);
  const fileEntity=entities[0]||null;
  if(fileEntity && fileEntity.toUpperCase()!==contextEntity.toUpperCase()) throw new Error(`ETS incohérent : le fichier YFDR contient ${fileEntity}, alors que l’import est contextualisé sur ${contextEntity}.`);
  for(const row of rows) row.establishment=contextEntity;
  return {type:'fdr',establishment:contextEntity,fileEntity,entityFromContext:!fileEntity,snapshotDate:new Date().toISOString().slice(0,10),rows,sourceRows:records.length};
}
...
app.post('/api/import/opale',async(req:any,reply:any)=>{try{
 ...
 try{
  ...
  if(name.endsWith('.csv') && detectCsvType(buf)==='fdr'){const contextEntity=String(targetEstablishment.opale_entity||requestedEts).trim();const p=parseFdr(buf,contextEntity); await client.query('begin'); const s=(await client.query('insert into fdr_snapshots(establishment_name,snapshot_date,source_filename,row_count) values($1,$2,$3,$4) returning *',[targetEstablishment.name,p.snapshotDate,file.filename,p.rows.length])).rows[0]; for(const r of p.rows)await client.query('insert into fdr_lines(snapshot_id,exercise,amount,direction,is_final,establishment,state,source_modified_at) values($1,$2,$3,$4,$5,$6,$7,$8)',[s.id,r.exercise,r.amount,r.direction,r.isFinal,r.establishment,r.state,r.sourceModifiedAt]); await client.query('commit'); return {ok:true,type:'fdr',snapshot:s,control:{sourceRows:p.sourceRows,importedRows:p.rows.length,etsFromContext:p.entityFromContext}}}
