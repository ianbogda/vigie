...
app.get('/api/financial/:ets',async(req:any,reply:any)=>{try{
 ...
 for(const snap of eblcAnnual){const q=(await pool.query(`select coalesce(sum(case when account ~ '^[67]' then credit-debit else 0 end),0) result,coalesce(sum(case when account='5151' or account like '5151%' then debit-credit else 0 end),0) treasury,coalesce(sum(case when account ~ '^6' then debit-credit else 0 end),0) operating_charges from financial_balance_lines where snapshot_id=$1`,[snap.id])).rows[0];const year=Number(snap.exercise),f=fdrByYear.get(year),treasury=Number(q.treasury||0),result=Number(q.result||0),operatingCharges=Number(q.operating_charges||0),fdrAmount=f?Number(f.amount):null,fdrDays=fdrAmount!=null&&operatingCharges>0?fdrAmount/(operatingCharges/365):null;indicatorHistory.push({exercise:year,snapshotDate:snap.snapshot_date,periodStart:snap.period_start||null,periodEnd:snap.period_end||snap.period||null,isCurrent:year===currentYear,isFinal:year<currentYear&&String(snap.period_end||snap.period||'').startsWith('12/'),fdr:fdrAmount,fdrFinal:f?!!f.is_final:false,fdrDays,operatingCharges,treasury,result,bfr:fdrAmount==null?null:fdrAmount-treasury})}
