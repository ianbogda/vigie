...
export async function treasuryContext(pool: Pool, snapshot: any) {
  ...
  const rows = (
    await pool.query(
      `select period,period_date,journal,debit,credit,movement,movement_kind from accounting_lines where opale_entity=$1 and account like '5151%' order by period_date,journal`,
      [entity]
    )
  ).rows;
  ...
  if (current && current.balance < 0)
    signals.push({
      ...
      domain: 'Trésorerie',
      processCode: 'TRE',
      title: 'Solde comptable 5151 négatif',
      detail: `Le solde comptable reconstitué ressort à ${current.balance.toFixed(2)} €.`,
      amount: current.balance,
      source: 'Mouvements 5151 Op@le',
      condition: 'Solde reconstitué < 0 €',
  ...
  return {
    ...
    currentBalance: currentSeries?.currentBalance ?? null,
    currentMovement: current?.movement ?? null,
    minBalance: currentSeries?.minBalance ?? null,
    maxBalance: currentSeries?.maxBalance ?? null,
    history: currentSeries?.history || [],
    series,
    signals,
    warning:
      'Solde comptable reconstitué par exercice : ZOUVER fixe le solde au 1er janvier ; les autres écritures font varier le 5151 au débit (+) et au crédit (−).'
  };
}
