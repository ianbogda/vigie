...
export async function treasuryContext(pool:Pool,snapshot:any){
 ...
 const entity=String(snapshot.opale_entity||snapshot.source_key||'');if(!entity)return null;
 const rows=(await pool.query(`select period,period_date,journal,debit,credit,movement,movement_kind from accounting_lines where opale_entity=$1 and account like '5151%' order by period_date,journal`,[entity])).rows;if(!rows.length)return null;
