...
async function parseInput(buf, filename) {
    ...
    if (ext === 'xls')
        throw new Error('Le format .xls n’est pas accepté. Enregistrez le fichier en .xlsx ou .csv puis réessayez.');
    if (!['xlsx', 'csv'].includes(ext || ''))
        throw new Error('Format non accepté. Formats autorisés : .xlsx et .csv.');
    let sheet = 'CSV';
    let rawRows = [];
    if (ext === 'csv') {
