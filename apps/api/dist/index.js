...
app.get('/api/dashboard', async () => {
    ...
    const map = new Map();
    const ensure = (key, name) => { if (!map.has(key))
        map.set(key, { key, name: name || key, sources: { balance: null, budget: null, purchases: null, fdr: null }, signals: [] }); return map.get(key); };
    for (const x of balances.rows) {
        const e = ensure(x.source_key, x.opale_entity_label || x.establishment_name);
        e.name = x.opale_entity_label || e.name;
        e.sources.balance = x;
    }
    ...
    for (const e of map.values()) {
        const states = { budget: 'missing', financial: 'missing', recovery: 'missing', suppliers: 'missing', accounting: 'missing' };
        if (e.sources.budget) {
    ...
    try {
        const pcif = (await pool.query('select establishment_key,campaign_label,mastery_level,open_actions,major_risks,updated_at from pcif_context')).rows;
        for (const e of establishments) {
            e.pcif = pcif.find((p) => p.establishment_key === e.id) || null;
        }
    }
    catch { }
